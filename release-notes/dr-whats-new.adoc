...
* * 支援內部部署到內部部署的 VMware VMFS FC 資料存放區 * ：此版本包含對安裝在 VMware vSphere 虛擬機器檔案系統（ VMFS ）資料存放區上的虛擬機器（ VM ）支援的技術預覽（ _Technology preview_ ）、以供 FC 保護到內部部署儲存設備。此服務先前提供技術預覽、支援 iSCI 的 VMFS 資料存放區。在此版本中、 BlueXP 災難恢復功能可支援內部部署 VMware 工作負載的技術預覽、以及內部部署的 VMware 環境、以及 FC 的 VMFS 資料存放區。
+
